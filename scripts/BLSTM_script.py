...
# Load the model
selected_model = input("Choose BiLSTM model (1 for model trained on data 'city_day.csv', 2 for model trained on data 'taiwan2015.csv'): ")
true_value_dict = {}

if selected_model == '1':
    model_path = 'SAVED_MODEL/BLSTM_model_city_day.h5'
    data_folder = 'input_files/city_day/LSTM'
    ...
    model_path = 'SAVED_MODEL/BLSTM_model_taiwan.h5'
    data_folder = 'input_files/taiwan2015/LSTM'
    true_value_dict = {0: 'Good', 
                           1: 'Moderate', 
                           2: 'Satisfactory'}
...
BLSTM_model = load_model(model_path)
...
def preprocess_input(text, selected):
    ...
    if selected == '1':
        data = data.reshape(1, 1, 12)  # Reshape for model trained on data 1
    elif selected == '2':
        data = data.reshape(1, 24, 6)   # Reshape for model trained on data 2
    return data

# Function to make predictions
def predict(input_data):
    # Make predictions
    predictions = BLSTM_model.predict(input_data)
    return predictions
...
def main():
    ...
    user_input = input("Would you like to continue evaluating with the current script - BLSTM ? (yes/no): ")
