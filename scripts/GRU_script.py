...
# Load the model
selected_model = input("Choose GRU model (1 for model trained on data 'city_day.csv', 2 for model trained on data 'taiwan2015.csv'): ")
true_value_dict = {}

if selected_model == '1':
    model_path = 'SAVED_MODEL/GRU_model_city_day.h5'
    ...
    model_path = 'SAVED_MODEL/GRU_model_taiwan.h5'
...
def predict(input_data):
    # Make predictions
    predictions = GRU_model.predict(input_data)
    return predictions
...
def main():
    ...
    user_input = input("Would you like to continue evaluating with the current script - GRU ? (yes/no): ")
